...
namespace neu {
	...
	void PostProcessComponent::UpdateGUI() {
		...
		parameter = iparameters & (uint32_t)Parameters::Noise;
		if (ImGui::Checkbox("Noise", &parameter)) {
			if (parameter) iparameters |= (uint32_t)Parameters::Noise;
			else iparameters &= ~(uint32_t)Parameters::Noise;
		}
